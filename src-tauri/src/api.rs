...
// Fetch Default Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<DefaultPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: DefaultPromptsResponse = response
        .json()
        .await
