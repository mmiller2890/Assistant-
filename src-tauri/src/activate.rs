...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "license_key" => storage.license_key = Some(item.value),
            "instance_id" => storage.instance_id = Some(item.value),
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "license_key" => storage.license_key = None,
            "instance_id" => storage.instance_id = None,
