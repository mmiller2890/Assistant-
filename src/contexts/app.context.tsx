...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // Pluely API State (hosted API removed — always disabled)
  ...
  const loadData = () => {
    ...
    // Load Pluely API enabled state (hosted API removed — always disabled)
  ...
  useEffect(() => {
    const checkImageSupport = async () => {
      // Hosted Pluely API has been removed; always evaluate custom provider image support.
  ...
  const setPluelyApiEnabled = async (_enabled: boolean) => {
    // Hosted Pluely API has been removed; this setter is retained as a no-op for compatibility.
    setPluelyApiEnabledState(false);
  };
